...
5. Generate the image.
   - Generate a raster image by default. Do not stop at prompt-only unless the user explicitly asks for that.
   - Use whichever image tool the current platform provides:
     - **Cursor:** `GenerateImage` with the compiled prompt as `description`. Prefer aspect ratio closest to tall poster (`3:4` if `3:5` is unavailable).
     - **Codex:** built-in image generation from the compiled prompt.
     - **Claude (Claude Code / Claude.ai agents):** native image generation when available; otherwise return the compiled prompt and ask the user to run it in their image tool.
...
This skill is meant to work the same on **Codex**, **Cursor**, and **Claude**. The visual rules and prompt compiler do not change by platform — only the image-generation call does.

- Invoke by attaching or naming `gc-minimal-zine-poster-v0-1` / `$gc-minimal-zine-poster-v0-1`.
- Keep outputs identical: image + final prompt + short recipe note.
